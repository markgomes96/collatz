...
do while (num > 0)      !Collatz algorithm
    n = num
    count = RecCount(n)      !Recursion
...
end program collatzrec

recursive integer(kind=8) function RecCount(n) result(count)
integer(kind=8) :: n
...
if(n == 1) then
    ...
    if(mod(n,2) == 0) then 
        count = 1 + RecCount(n/2)
    else
        count = 1 + RecCount(((n*3)+1))
    end if
endif

end function RecCount
